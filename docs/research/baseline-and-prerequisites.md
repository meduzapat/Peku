...
| **D-03** `[E]` | **Confirmed by the suite: the 4 `ErrorHandlerTest` errors are `MockDelegateFunction::delegate() was not expected to be called more than once`.** **`handleFatal()` re-logs the last non-fatal error at shutdown.** `error_clear_last()` (`:73`) is defeated by `return false` (`:74`), which hands control to PHP's internal handler, which re-populates the last error. Reproduced: a single warning logged twice, once at the call site and once at shutdown |
